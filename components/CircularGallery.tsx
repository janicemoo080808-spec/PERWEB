...
function createTextTexture(gl: any, text: string, font = 'bold 24px monospace', color = 'white') {
  ...
  context.roundRect(5, 5, canvas.width - 10, canvas.height - 10, r);
  context.fill();
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#ffffff', font = 'bold 24px monospace' }: any) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class Media {
  ...
  update(scroll: any, direction: string) {
    ...
    this.speed = (scroll.current - scroll.last) * 0.1;
